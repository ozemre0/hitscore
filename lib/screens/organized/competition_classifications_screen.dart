...
class CompetitionClassificationsScreen extends StatefulWidget {
  ...
  const CompetitionClassificationsScreen({super.key, required this.competitionId});
...
class _CompetitionClassificationsScreenState extends State<CompetitionClassificationsScreen> {
  ...
  Future<void> _saveClassifications() async {
    if (_classifications.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('En az bir klasman eklemelisiniz')));
      return;
    }
    setState(() => _isLoading = true);
    try {
      await SupabaseConfig.client
          ...
          .eq('organized_competition_id', widget.competitionId);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Yarışma başarıyla oluşturuldu!')));
                  ...
                  Card(
                    child: Padding(
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        const SizedBox(height: 8),
                        Text('Tarih: ${_formatDate(_competitionData!['competition_date'])}', style: Theme.of(context).textTheme.bodySmall),
                      ]),
                    ),
                  ),
                  ...
                  ...List.generate(_classifications.length, (index) {
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        subtitle: Text('${classification['ageGroup']} • ${classification['bowType']} • ${classification['distance']}m • ${classification['environment']}'),
...
class _ClassificationDialogState extends State<_ClassificationDialog> {
  ...
  final List<String> _ageGroups = ['9-10 Yaş', '11-12 Yaş', '13-14 Yaş', 'U18 (15-16-17)', 'U21 (18-19-20)', 'Büyükler'];
  final List<String> _bowTypes = ['Recurve', 'Compound', 'Barebow'];
  final List<String> _environments = ['Indoor', 'Outdoor'];
  ...
  void _save() {
    ...
    if (distance == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Geçerli bir mesafe girin')));
      return;
    }
    widget.onSave({
      ...
      content: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(
            ...
            decoration: const InputDecoration(labelText: 'Klasman Adı', border: OutlineInputBorder(), hintText: 'Örn: Büyükler Erkek Recurve 70m'),
          ),
          const SizedBox(height: 16),
          DropdownButtonFormField<String>(
            value: _selectedAgeGroup,
            decoration: const InputDecoration(labelText: 'Yaş Grubu', border: OutlineInputBorder()),
            items: _ageGroups.map((age) => DropdownMenuItem(value: age, child: Text(age))).toList(),
            onChanged: (value) => setState(() => _selectedAgeGroup = value),
          ),
          const SizedBox(height: 16),
          DropdownButtonFormField<String>(
            value: _selectedBowType,
            decoration: const InputDecoration(labelText: 'Yay Tipi', border: OutlineInputBorder()),
          ...
          DropdownButtonFormField<String>(
            ...
            decoration: const InputDecoration(labelText: 'Ortam', border: OutlineInputBorder()),
          ...
          DropdownButtonFormField<int>(
            ...
            decoration: const InputDecoration(labelText: 'Mesafe (metre)', border: OutlineInputBorder()),
            items: [
              ..._distances.map((d) => DropdownMenuItem(value: d, child: Text('${d}m'))),
              const DropdownMenuItem(value: -1, child: Text('Özel Mesafe')),
            ],
            onChanged: (value) => setState(() => _selectedDistance = value == -1 ? null : value),
          ),
          if (_selectedDistance == null) ...[
            const SizedBox(height: 16),
            TextField(controller: _customDistanceController, decoration: const InputDecoration(labelText: 'Özel Mesafe (metre)', border: OutlineInputBorder()), keyboardType: TextInputType.number),
          ],
        ]),
      ),
      actions: [
